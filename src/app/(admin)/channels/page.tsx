...
export default function ChannelsPage() {
  ...
  return (
    <PageShell>
      ...
      <DataTableShell title="Chaines TV" description="Catalogue editorial, identite visuelle, readiness playback et actions operateur." loading={loading && rows.length === 0} error={!rows.length ? pageError || undefined : undefined} onRetry={() => void loadData()} isEmpty={!pageError && filteredRows.length === 0} emptyTitle="Aucune chaine" emptyDescription="Aucune chaine ne correspond aux filtres actifs." emptyAction={<Button onClick={startCreate}><Plus className="size-4" />Creer une chaine</Button>} footer={<div className="flex flex-col gap-2 text-xs uppercase tracking-[0.14em] text-slate-500 sm:flex-row sm:items-center sm:justify-between"><span>{filteredRows.length} ligne(s) affichee(s)</span><span>{stats.withOrigin} origine(s) prêtes | {stats.active} chaine(s) actives</span></div>}>
        <Table>
