...
const CATS = ["Actualite", "Sports", "Music", "Religion", "Films et Series", "Documentaire", "Art", "Mode", "Faits Divers", "Anime", "Manga", "Autre"] as const;
...
function Health({ status }: { status: "ok" | "degraded" | "down" | null }) {
  ...
  if (status === "degraded") return <Badge className="border-amber-500/25 bg-amber-500/10 text-amber-200">Degrade</Badge>;
  if (status === "down") return <Badge className="border-rose-500/25 bg-rose-500/10 text-rose-200">Down</Badge>;
  return <Badge variant="outline">Non controle</Badge>;
}
...
export default function ChannelsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setPageError(err(error, "Impossible de charger le catalogue des chaines."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const loadChannelStats = useCallback(async (channelId: string) => {
    ...
    try {
      ...
      setStatsError(err(error, "Impossible de charger les stats OTT."));
    } finally {
  ...
  const save = async () => {
    ...
    try {
      ...
      setFormError(err(error, "Impossible d enregistrer la chaine."));
    } finally {
      setBusy(false);
    }
  };
  ...
  const onToggle = async (channel: Channel) => {
    ...
    try {
      ...
      setRows((prev) => prev.map((row) => (row.id === channel.id ? { ...row, active: !nextActive } : row)));
      setPageError(err(error, "Impossible de modifier le statut de la chaine."));
    }
  };
  ...
  return (
    <PageShell>
      <PageHeader
        title="Catalogue chaines"
        subtitle="Pilotage editorial, readiness OTT et exposition des chaines depuis une seule surface d exploitation."
        breadcrumbs={[{ label: "Oniix Console", href: "/dashboard" }, { label: "Catalogue" }, { label: "Chaines TV" }]}
        icon={<Tv className="size-5" />}
        actions={
          <>
            <Button variant="outline" onClick={() => void loadData()}><RefreshCw className={cn("size-4", loading && "animate-spin")} />Actualiser</Button>
            <Button onClick={startCreate}><Plus className="size-4" />Nouvelle chaine</Button>
          </>
        }
      />

      <KpiRow>
        <KpiCard label="Catalogue" value={stats.total} hint="Chaines configurees" icon={<Tv className="size-4" />} loading={loading && rows.length === 0} />
        ...
        <KpiCard label="Origines pretes" value={stats.withOrigin} hint={`${stats.missingOrigin} a raccorder`} icon={<Link2 className="size-4" />} tone={stats.missingOrigin > 0 ? "warning" : "info"} loading={loading && rows.length === 0} />
        <KpiCard label="Categories" value={stats.categories} hint="Segmentation editoriale" icon={<Activity className="size-4" />} loading={loading && rows.length === 0} />
      </KpiRow>
      ...
      {stats.missingOrigin > 0 ? <div className="rounded-[24px] border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">{stats.missingOrigin} chaine(s) n ont pas encore d origine HLS. Tant que ce point reste ouvert, la gateway ne peut pas servir un playback propre.</div> : null}

      <Card>
        <CardHeader>
          <CardTitle>Pilotage catalogue</CardTitle>
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_220px_220px_auto]">
            <div className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-slate-500" /><Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Rechercher par nom ou slug" className="pl-11" /></div>
            <Select value={catFilter} onValueChange={setCatFilter}><SelectTrigger className="w-full"><SelectValue placeholder="Categorie" /></SelectTrigger><SelectContent><SelectItem value="ALL">Toutes categories</SelectItem>{CATS.map((category) => <SelectItem key={category} value={category}>{category}</SelectItem>)}</SelectContent></Select>
            <Select value={originFilter} onValueChange={(value) => setOriginFilter(value as OriginFilter)}><SelectTrigger className="w-full"><SelectValue placeholder="Origine HLS" /></SelectTrigger><SelectContent><SelectItem value="ALL">Toutes origines</SelectItem><SelectItem value="CONFIGURED">Origine configuree</SelectItem><SelectItem value="MISSING">Origine manquante</SelectItem></SelectContent></Select>
            <Button variant="outline" onClick={resetFilters}>Reinitialiser</Button>
          </div>
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="inline-flex rounded-[18px] border border-white/10 bg-white/[0.03] p-1">{[{ label: "Tous", value: "ALL" }, { label: "Actives", value: "ACTIVE" }, { label: "Inactives", value: "INACTIVE" }].map((item) => <button key={item.value} type="button" onClick={() => setActiveFilter(item.value as ActiveFilter)} className={cn("rounded-[14px] px-3 py-2 text-xs font-semibold transition", activeFilter === item.value ? "bg-white text-slate-950" : "text-slate-400 hover:text-white")}>{item.label}</button>)}</div>
            <div className="flex flex-wrap items-center gap-2"><Badge variant="secondary">{filteredRows.length} resultat(s)</Badge><Badge variant="outline">{stats.total} chaine(s)</Badge>{stats.missingOrigin > 0 ? <Badge className="border-amber-500/25 bg-amber-500/10 text-amber-200">{stats.missingOrigin} origine(s) manquante(s)</Badge> : null}</div>
          </div>
        </CardContent>
      </Card>

      <DataTableShell title="Chaines TV" description="Catalogue editorial, identite visuelle, readiness playback et actions operateur." loading={loading && rows.length === 0} error={!rows.length ? pageError || undefined : undefined} onRetry={() => void loadData()} isEmpty={!pageError && filteredRows.length === 0} emptyTitle="Aucune chaine" emptyDescription="Aucune chaine ne correspond aux filtres actifs." emptyAction={<Button onClick={startCreate}><Plus className="size-4" />Creer une chaine</Button>} footer={<div className="flex flex-col gap-2 text-xs uppercase tracking-[0.14em] text-slate-500 sm:flex-row sm:items-center sm:justify-between"><span>{filteredRows.length} ligne(s) affichee(s)</span><span>{stats.withOrigin} origine(s) pretes | {stats.active} chaine(s) actives</span></div>}>
        <Table>
          <TableHeader><TableRow><TableHead>Chaine</TableHead><TableHead>Categorie</TableHead><TableHead>Slug</TableHead><TableHead>Origine HLS</TableHead><TableHead>Statut</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {filteredRows.map((channel) => <TableRow key={channel.id}><TableCell><div className="flex min-w-0 items-center gap-3"><Avatar className="size-11 rounded-[18px] border border-white/10 bg-white/[0.04]"><AvatarImage src={channel.logo || ""} className="object-cover" /><AvatarFallback className="rounded-[18px] bg-white/[0.06] text-xs font-semibold text-slate-300">{channel.name.slice(0, 2).toUpperCase()}</AvatarFallback></Avatar><div className="min-w-0"><div className="truncate font-medium text-white">{channel.name}</div><div className="text-[11px] uppercase tracking-[0.14em] text-slate-500">{channel.id.slice(0, 8)}</div></div></div></TableCell><TableCell><Badge variant="outline">{channel.category}</Badge></TableCell><TableCell><code className="rounded-[12px] border border-white/10 bg-white/[0.03] px-2.5 py-1 text-xs text-slate-300">/{channel.slug}</code></TableCell><TableCell>{channel.originHlsUrl ? <Badge className="border-emerald-500/25 bg-emerald-500/10 text-emerald-200">Configuree</Badge> : <Badge className="border-amber-500/25 bg-amber-500/10 text-amber-200">Manquante</Badge>}</TableCell><TableCell>{channel.active ? <Badge className="border-emerald-500/25 bg-emerald-500/10 text-emerald-200">Active</Badge> : <Badge variant="outline">Inactive</Badge>}</TableCell><TableCell className="text-right"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" title="Actions"><MoreHorizontal className="size-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48"><DropdownMenuItem onClick={() => startEdit(channel)}><Edit3 className="size-4" />Modifier</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => void onToggle(channel)}><Power className="size-4" />{channel.active ? "Desactiver" : "Activer"}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}
          </TableBody>
        </Table>
      </DataTableShell>
        ...
        <SheetContent side="right" className="w-full max-w-[760px] sm:max-w-[760px]">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto p-6">
            <section className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2"><Label>Categorie</Label><Select value={(form.category as string | undefined) || "Autre"} onValueChange={(value) => setForm((prev) => ({ ...prev, category: value as Category }))}><SelectTrigger className="w-full"><SelectValue placeholder="Choisir une categorie" /></SelectTrigger><SelectContent>{CATS.map((category) => <SelectItem key={category} value={category}>{category}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-2"><div className="flex items-center justify-between"><Label>Slug</Label><Button type="button" variant="ghost" size="sm" onClick={() => setForm((prev) => ({ ...prev, slug: slugify(prev.name || "") }))}>Generer</Button></div><Input value={form.slug || ""} onChange={(event) => setForm((prev) => ({ ...prev, slug: event.target.value }))} placeholder="oniix-sports-1" /></div>
            ...
            <section className="rounded-[24px] border border-white/8 bg-white/[0.03] p-5">
              <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-white">Distribution OTT</p><p className="mt-1 text-sm text-slate-400">URL HLS d origine editeur. La lecture publique passe ensuite par la gateway.</p></div><Badge variant="outline">Playback</Badge></div>
              ...
              <div className="mt-4 flex items-center justify-between rounded-[18px] border border-white/8 bg-black/10 px-4 py-3"><div><p className="text-sm font-medium text-white">Publication</p><p className="mt-1 text-xs text-slate-400">Activez la chaine quand l origine et le branding sont valides.</p></div><Button type="button" variant="outline" onClick={() => setForm((prev) => ({ ...prev, active: !Boolean(prev.active) }))}>{form.active ? "Desactiver" : "Activer"}</Button></div>
            </section>

            {editing ? <section className="space-y-4">
              <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-white">Observabilite OTT</p><p className="mt-1 text-sm text-slate-400">KPIs quasi temps reel et sante de diffusion pour cette chaine.</p></div><Button variant="outline" size="sm" onClick={() => void loadChannelStats(editing.id)}><RefreshCw className={cn("size-4", isStatsLoading && "animate-spin")} />Actualiser</Button></div>
