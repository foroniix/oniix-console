...
export async function GET() {
  try {
    ...
    const publicationByPlayableId = new Map(activePublications.map((publication) => [publication.playable_id, publication]));
    const hasExplicitPublication = activePublications.length > 0;
    const titleIds = uniqueIds(activePublications.map((publication) => publication.playable_id));

    let titleRows: Record<string, unknown>[] = [];
    if (hasExplicitPublication) {
      ...
      if (error) {
        console.error("Web catalog titles load error", { error: error.message });
        return NextResponse.json({ ok: false, error: "Une erreur est survenue." }, { status: 500 });
      }

      titleRows = (data ?? []) as Record<string, unknown>[];
    } else {
      const { data, error } = await admin
        .from("catalog_titles")
        .select(CATALOG_TITLE_SELECT)
        .in("title_type", ["movie", "series"])
        .in("editorial_status", ["ready", "published"])
        .order("updated_at", { ascending: false })
        .limit(2000);

      if (error) {
        console.error("Web catalog fallback titles load error", { error: error.message });
        return NextResponse.json({ ok: false, error: "Une erreur est survenue." }, { status: 500 });
      }
    ...
    const titles = titleRows.map((row) => normalizeCatalogTitleRow(row));
    const visibleTitleIds = titles.map((title) => title.id);
    if (visibleTitleIds.length === 0) {
    ...
    const episodePlayableIds = new Set(episodeSourceRows.map((source) => source.playable_id));
    const seriesPlayableIds = new Set(
      Array.from(episodesBySeries.entries())
        .filter(([, ids]) => ids.some((episodeId) => episodePlayableIds.has(episodeId)))
        .map(([seriesId]) => seriesId)
    );

    const items = titles
      .map((title) => {
        const publication = publicationByPlayableId.get(title.id) ?? null;
        ...
        return {
          ...
          storefront: publication?.storefront ?? "public-web",
          featured_rank: publication?.featured_rank ?? null,
    ...
    console.error("Web catalog route crash", {
