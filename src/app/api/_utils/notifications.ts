...
export async function syncSystemNotifications(ctx: AuthContext) {
  ...
  if (role === "superadmin") {
    ...
    if ((missingOrigin ?? 0) > 0 || (missingTenant ?? 0) > 0) {
      await syncNotification(admin, ctx.userId, {
        ...
        severity: "warning",
        title: "Remédiation catalogue requise",
        body: `${missingOrigin ?? 0} chaîne(s) sans origine HLS et ${missingTenant ?? 0} chaîne(s) sans rattachement éditeur bloquent encore une partie du catalogue.`,
        actionLabel: "Ouvrir la remédiation",
    ...
    if (tenantsTotal !== null && ingestConfigured !== null && ingestConfigured < tenantsTotal) {
      await syncNotification(admin, ctx.userId, {
        ...
        body: `${tenantsTotal - ingestConfigured} éditeur(s) n'ont pas encore de configuration ingest active.`,
  ...
  if ((healthAlerts ?? 0) > 0) {
    await syncNotification(admin, ctx.userId, {
      ...
      body: `${healthAlerts} chaîne(s) présentent actuellement un état dégradé ou indisponible côté monitoring.`,
