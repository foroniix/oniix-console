...
export async function syncSystemNotifications(ctx: AuthContext) {
  ...
  await syncNotification(admin, ctx.userId, {
    ...
    body: "Une question produit, un incident ou un besoin d'onboarding ? Ecrivez a support@oniix.space.",
  ...
  if (role === "superadmin") {
    ...
    if ((missingOrigin ?? 0) > 0 || (missingTenant ?? 0) > 0) {
      await syncNotification(admin, ctx.userId, {
        ...
        severity: "warning",
        title: "Remediation catalogue requise",
        body: `${missingOrigin ?? 0} chaine(s) sans origine HLS et ${missingTenant ?? 0} chaine(s) sans rattachement editeur bloquent encore une partie du catalogue.`,
        actionLabel: "Ouvrir la remediation",
    ...
    if (tenantsTotal !== null && ingestConfigured !== null && ingestConfigured < tenantsTotal) {
      await syncNotification(admin, ctx.userId, {
        ...
        severity: "warning",
        title: "Couverture ingest incomplete",
        body: `${tenantsTotal - ingestConfigured} editeur(s) n'ont pas encore de configuration ingest active.`,
        actionLabel: "Verifier la plateforme",
        actionUrl: "/system",
  ...
  if ((missingOrigin ?? 0) > 0) {
    await syncNotification(admin, ctx.userId, {
      ...
      severity: "warning",
      title: "Chaines sans origine HLS",
      body: `${missingOrigin} chaine(s) de votre espace n'ont pas encore d'origine HLS et ne seront pas lisibles dans l'application.`,
      actionLabel: "Ouvrir les chaines",
      actionUrl: "/channels",
  ...
  if ((healthAlerts ?? 0) > 0) {
    await syncNotification(admin, ctx.userId, {
      ...
      severity: "critical",
      title: "Flux a surveiller",
      body: `${healthAlerts} chaine(s) presentent actuellement un etat degrade ou indisponible cote monitoring.`,
      actionLabel: "Voir les chaines",
      actionUrl: "/channels",
  ...
  if (ingestConfigured === 0) {
    await syncNotification(admin, ctx.userId, {
      ...
      severity: "info",
      title: "Ingest mobile a finaliser",
      body: "La cle d'ingest mobile n'est pas encore configuree pour cet espace. Les analytics applicatives resteront incompletes.",
