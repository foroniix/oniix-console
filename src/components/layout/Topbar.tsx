...
export default function Topbar() {
  ...
  const currentPage = ROUTE_NAMES[pathname] || pathname.split("/").pop() || "Dashboard";
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/5 bg-zinc-950/80 px-6 backdrop-blur-md transition-all">
        
        {/* --- FIL D'ARIANE --- */}
        <div className="flex items-center gap-2 text-sm text-zinc-500">
          <span className="hover:text-zinc-300 transition-colors cursor-pointer" onClick={() => router.push('/')}>Oniix</span>
          <ChevronRight className="h-4 w-4 text-zinc-700" />
          <span className="font-medium text-zinc-100">{currentPage}</span>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-white hover:bg-white/5 h-9 w-9 relative">
                  <Bell className="h-5 w-5" />
                  {NOTIFICATIONS.length > 0 && (
                    <span className="absolute top-2 right-2 h-2 w-2 rounded-full bg-rose-500 border border-zinc-950 animate-pulse"></span>
                  )}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-80 bg-zinc-950 border-zinc-800 text-zinc-100">
                <DropdownMenuLabel className="flex justify-between items-center">
                  Notifications
                  <Badge variant="outline" className="text-[10px] border-zinc-700 text-zinc-400">{NOTIFICATIONS.length} nouvelles</Badge>
                </DropdownMenuLabel>
                <DropdownMenuSeparator className="bg-zinc-800" />
                <div className="max-h-[300px] overflow-y-auto">
                  {NOTIFICATIONS.map((notif) => (
                    <DropdownMenuItem key={notif.id} className="cursor-pointer focus:bg-zinc-900 p-3 flex flex-col items-start gap-1 border-b border-zinc-900 last:border-0">
                      <div className="flex justify-between w-full items-center">
                        <span className={`text-xs font-bold ${notif.type === 'error' ? 'text-rose-400' : notif.type === 'success' ? 'text-emerald-400' : 'text-blue-400'}`}>
                          {notif.title}
                        </span>
                        <span className="text-[10px] text-zinc-500">{notif.time}</span>
                      </div>
                      <span className="text-xs text-zinc-400">{notif.desc}</span>
                    </DropdownMenuItem>
                  ))}
                </div>
                <DropdownMenuSeparator className="bg-zinc-800" />
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-white hover:bg-white/5 h-9 w-9">
                  <HelpCircle className="h-5 w-5" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="bg-zinc-950 border-zinc-800 text-zinc-100">
                <DropdownMenuItem className="cursor-pointer focus:bg-zinc-900">Documentation</DropdownMenuItem>
                <DropdownMenuItem className="cursor-pointer focus:bg-zinc-900">Raccourcis Clavier</DropdownMenuItem>
                <DropdownMenuItem className="cursor-pointer focus:bg-zinc-900 text-indigo-400">Contacter le Support</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <DialogContent className="p-0 gap-0 bg-zinc-950 border-zinc-800 text-zinc-100 sm:max-w-[550px] overflow-hidden shadow-2xl shadow-black/50">
          ...
          <div className="max-h-[300px] overflow-y-auto p-2">
            {filteredItems.length === 0 ? (
              ...
              <>
                <div className="mb-2 px-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Navigation Rapide</div>
            ...
            <div className="mt-4 mb-2 px-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest border-t border-white/5 pt-2">Actions</div>
            <div 
